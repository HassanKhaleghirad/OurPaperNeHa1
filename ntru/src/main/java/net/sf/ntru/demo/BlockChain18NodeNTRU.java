...
public class BlockChain18NodeNTRU {
    public static void main(String[] args) throws Exception {
        ...
        for (int i=1 ; i<6;i++){
            myNode = new Node();
            JSONObject transaction2 = aesExample1.run("node"+i);
            EncryptionKeyPair transaction_temp2 = (EncryptionKeyPair) transaction2.get(1);
            Block block2 = new Block(i+1, transaction_temp2.toString(), blockChainNTRU.getBlockChain().get(blockChainNTRU.size() - 1).getHash());
            miner.mine(block2, blockChainNTRU);
            byte[] encrypt_temp2 = (byte[]) transaction2.get(2);
            myNode.decrypt(encrypt_temp2, transaction_temp2);
        }
